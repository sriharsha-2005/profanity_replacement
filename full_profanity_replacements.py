...
def process_dataset(input_file: str, output_file: str):
    with open(input_file, 'r', encoding='utf-8') as infile, open(output_file, 'w', newline='', encoding='utf-8') as outfile:
        ...
        writer = csv.DictWriter(outfile, fieldnames=['badword', 'type', 'replacement_word', 'replacement_word_type'])
        writer.writeheader()
        for row in reader:
            original_word = row['badword'].strip().lower()
            word_type = row['type'].strip().lower()
            if not original_word or not word_type:
                continue
...
if __name__ == "__main__":
    process_dataset("processed_bad_words_fast.csv", "all_clean_replacements.csv")